...
void Hydro::doCycle(
    ...
    RAJA::forall<chunk_policy>(0, mesh->numzch, [=] (int zch) {
        ...
        calcDtHydro(zdl, zvol, zvol0, dt, zfirst, zlast, newdt);
    });  // for zch
    dtrec = newdt.get();
}
...
void Hydro::calcDtCourant(
    ...
    RAJA::ReduceMinLoc<inner_reduce_policy, double> dtnew(1.e99, -1);
    ...
    RAJA::forall<exec_policy>(zfirst, zlast, [=] RAJA_DEVICE(int z) {
        ...
        dtnew.minloc(zdthyd, z);
    });

    double dtn = dtnew.get();
    if (dtn < dtrec) {
        dtrec = dtn;
        snprintf(msgdtrec, 80, "Hydro Courant limit for z = %d", dtnew.getLoc());
    }
...
void Hydro::calcDtVolume(
    ...
    RAJA::forall<exec_policy>(zfirst, zlast, [=] RAJA_DEVICE(int z) {
        ...
        dvovmax.maxloc(zdvov, z);
    });

    double dtnew = dtlast * cflv / dvovmax.get();

    if (dtnew < dtrec) {
        dtrec = dtnew;
        snprintf(msgdtrec, 80, "Hydro dV/V limit for z = %d", dvovmax.getLoc());
    }
...
void Hydro::calcDtHydro(
        ...
        const int zfirst,
        const int zlast,
        RAJA::ReduceMin<reduce_policy, double> newdt) {
...
void Hydro::writeEnergyCheck() {
    ...
    double ei_ = ei.get();
    double ek_ = ek.get();
    Parallel::globalSum(ei_);
    Parallel::globalSum(ek_);
